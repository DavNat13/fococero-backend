...
/**
 * GeoRepository: La capa de acceso a datos estricta.
 * Única responsable de hablar el lenguaje SQL y PostGIS.
 */
export class GeoRepository {
    ...
    static async create(data: any): Promise<any> {
        const query = `
            ...
            RETURNING 
                ...
                radio_afectacion_metros, viento_velocidad_kmh, amenaza_viviendas,
                ST_Y(coordenadas::geometry) AS latitud,
                ST_X(coordenadas::geometry) AS longitud,
                created_at; 
        `;
        const values = [
            data.reporte_id, data.tipo_incidente, data.severidad, data.estado || 'Reportado', data.detalles,
            data.radio_afectacion_metros || 50, data.viento_velocidad_kmh || 0,
            data.viento_direccion, data.amenaza_viviendas || false,
    ...
    static async findNearby(lat: number, lng: number, radioMetros: number): Promise<any[]> {
        const query = `
            SELECT 
                id, reporte_id, tipo_incidente, severidad, estado, detalles,
                ST_Y(coordenadas::geometry) AS latitud,
    ...
    static async updateState(id: string, estado: string): Promise<any | null> {
        const query = `
    ...
    static async updatePerimetro(id: string, wktPoligono: string): Promise<any | null> {
        const query = `
            UPDATE public.ubicaciones 
            SET area_quemada_wkt = ST_GeomFromText($1, 4326)::geography 
            WHERE id = $2 AND deleted_at IS NULL
            RETURNING id, ST_AsText(area_quemada_wkt) AS nuevo_perimetro, updated_at;
        `;
        const { rows } = await pool.query(query, [wktPoligono, id]);
        return rows[0] || null;
    }

    static async updateAll(id: string, data: any): Promise<any | null> {
        const query = `
            UPDATE public.ubicaciones 
            SET 
            ...
            RETURNING id, tipo_incidente, severidad, updated_at;
        `;
        const values = [
            data.tipo_incidente, data.detalles, 
            data.viento_velocidad_kmh, data.amenaza_viviendas, 
            data.severidad, id
        ];
        const { rows } = await pool.query(query, values);
        return rows[0] || null;
